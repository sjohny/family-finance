...
import { Plus, Trash2, X, ChevronDown, RotateCcw, TrendingUp, TrendingDown } from 'lucide-react'
import {
...
function SetIncomeModal({ month, year, currentIncome, onClose }: {
  month: number; year: number; currentIncome: number; onClose: () => void
}) {
  const [loading, setLoading] = useState(false)
  const router = useRouter()
      ...
      <div className="modal-content">
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          <div>
            ...
            <input name="income" type="number" step="0.01" min="0" defaultValue={currentIncome || ''} placeholder="0.00" required />
          </div>
          <button type="submit" className="btn-primary" style={{ width: '100%', justifyContent: 'center' }} disabled={loading}>
            {loading ? 'Saving...' : 'Save'}
          </button>
...
export default function LedgerClient({
  transactions, subscriptions, totalIncome, totalExpenses, totalSubscriptions,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
        <div className="card animate-slide-up" style={{ padding: '1.25rem' }} data-delay="1">
          ...
          <p style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '1.25rem', color: 'var(--accent-green)' }}>
        ...
        <div className="card animate-slide-up" style={{ padding: '1.25rem' }} data-delay="2">
          ...
          <p style={{ fontFamily: 'var(--font-mono)', fontWeight: 700, fontSize: '1.25rem', color: 'var(--accent-red)' }}>
      ...
      {tab === 'expenses' && (
        <div className="card animate-fade-in" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontSize: '1.0625rem', fontWeight: 600 }}>
                ...
                return (
                  <div key={t.id} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{formatDate(t.date)} · {ci.label}</p>
                    </div>
                    <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, color: 'var(--accent-red)', flexShrink: 0, fontSize: '0.9375rem' }}>
                      -{formatCurrency(t.amount)}
                    </span>
                    <button
                      onClick={async () => { await deleteTransaction(t.id); router.refresh() }}
        ...
        <div className="card animate-fade-in" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <button className="btn-ghost" style={{ padding: '0.5rem 0.875rem', fontSize: '0.875rem' }} onClick={() => setModal('set_income')}>
                Set Expected
              </button>
              <button className="btn-primary" style={{ padding: '0.5rem 0.875rem', fontSize: '0.875rem' }} onClick={() => setModal('income')}>
                <Plus size={16} /> Add
              </button>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
            <div>
              ...
              <p style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '2px' }}>
                {formatCurrency(totalSubscriptions)}/mo total
              </p>
            </div>
                  ...
                  <div key={sub.id} style={{
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                        {ci.label} · Bills on day {sub.billingDay}
                      </p>
                    </div>
                    <span style={{ fontFamily: 'var(--font-mono)', fontWeight: 600, fontSize: '0.9375rem', flexShrink: 0 }}>
                      {formatCurrency(sub.amount)}/mo
                    </span>
                    <button
                      onClick={async () => { await deleteSubscription(sub.id); router.refresh() }}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '4px', flexShrink: 0 }}
                    >
                      <Trash2 size={15} />
                    </button>
